...
namespace Company
{
    public class TransportCompany
    {
        ...
        public Tariph FindCheapestTariph()
        {
            if(this.TariphList.Count == 0)
                return null;
            Tariph tariph = this.TariphList[0];
            foreach(Tariph tar in this.TariphList)
            {
                if (tar.GetTotalPrice() > tariph.GetTotalPrice())
                    tariph = tar;
            }
